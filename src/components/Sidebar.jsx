...
const NavItem = ({ active, label, onClick, children }) => (
  <button
    onClick={onClick}
    className={`w-full h-10 flex items-center gap-3 px-4 rounded-md text-sm font-medium transition-colors ${active ? 'bg-white/10 text-white' : 'text-gray-300 hover:bg-white/5 hover:text-white'}`}
  >
...
const Sidebar = ({ activeView, setActiveView, onCreateAction, open = false, onClose }) => {
  ...
  const CreateItem = ({ onClick, icon, label, color = 'bg-gray-100 text-gray-700' }) => (
    <button onClick={onClick} className="flex flex-col items-center px-3 py-2 hover:bg-gray-50 rounded-md">
      <span className={`h-8 w-8 rounded-md inline-flex items-center justify-center ${color}`}>{icon}</span>
      <span className="mt-1 text-xs font-semibold text-slate-700">{label}</span>
    </button>
  );
  const Body = (
    <div className="flex flex-col w-64 bg-slate-900 text-white min-h-full">
      <div className="px-4 py-4 border-b border-white/10 flex items-center gap-2">
        <div className="h-8 w-8 rounded bg-white/10 flex items-center justify-center font-bold">SH</div>
        <div className="text-sm opacity-80">Service Hub</div>
      </div>
      <div className="p-4 relative">
        <div className="mb-4 relative">
          <button
            onClick={() => setCreateOpen(v => !v)}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-sm font-semibold"
          >
          ...
          {createOpen && (
            <div className="absolute left-full ml-3 top-0 z-[9999]">
              {/* little caret */}
              <div className="absolute -left-2 top-3 h-4 w-4 bg-white border border-gray-200 rotate-45 rounded-sm" />
              <div className="relative bg-white text-slate-900 rounded-xl shadow-xl border border-gray-200 p-3">
                <div className="flex items-center gap-3">
                  <CreateItem label="Client" icon={<UsersIcon className="h-4 w-4" />} color="bg-slate-100 text-slate-700" onClick={() => { onCreateAction && onCreateAction('client'); setCreateOpen(false); }} />
                  <CreateItem label="Request" icon={<FileTextIcon className="h-4 w-4" />} color="bg-amber-100 text-amber-700" onClick={() => { onCreateAction && onCreateAction('request'); setCreateOpen(false); }} />
                  <CreateItem label="Quote" icon={<FileTextIcon className="h-4 w-4" />} color="bg-rose-100 text-rose-700" onClick={() => { onCreateAction && onCreateAction('quote'); setCreateOpen(false); }} />
                  <CreateItem label="Job" icon={<BriefcaseIcon className="h-4 w-4" />} color="bg-green-100 text-green-700" onClick={() => { onCreateAction && onCreateAction('job'); setCreateOpen(false); }} />
                  <CreateItem label="Invoice" icon={<InvoiceIcon className="h-4 w-4" />} color="bg-blue-100 text-blue-700" onClick={() => { onCreateAction && onCreateAction('invoice'); setCreateOpen(false); }} />
                </div>
              </div>
            </div>
          )}
